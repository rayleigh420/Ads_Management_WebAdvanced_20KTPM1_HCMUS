...
const routes: {
  ...
  { key: 'reports', label: 'Danh sách báo cáo', icon: 'ward' },
  {
    key: 'ads',
    label: 'Quản lý quảng cáo',
    icon: 'ads',
    children: [
      { key: MY_ROUTE.WARD.LICENSE, label: 'Danh sách cấp phép' },
      { key: MY_ROUTE.ADS.LOCATION, label: 'Danh sách điểm đặt quảng cáo' },
    ],
  },
];

export default function SiderMenuWard({ collapsed, setCollapsed }: SiderMenuProps) {
      ...
      >
        {routes.map((route) => {
          if (route.children) {
            return (
              <SubMenu
                ...
                icon={<MenuIcon icon={route.icon} isActive={isSubmenuActive(route.key)} />}
          ...
          return (
            <Item
              key={route.key}
              icon={<MenuIcon icon={route.icon} isActive={!!selectedKeys.includes(route.key)} />}
              onClick={() => {
