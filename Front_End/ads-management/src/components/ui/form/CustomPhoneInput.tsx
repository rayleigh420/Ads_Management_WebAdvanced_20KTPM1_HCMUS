...
import { CustomPhoneInputProps, PhoneNumber } from '../../utils/types/phone.type';

import countries from '@/utils/json/countries.json';
import timezones from '@/utils/json/timezones.json';
import validations from '@/utils/json/validations.json';
...
const checkValidity = (metadata: PhoneNumber, strict: boolean = false) => {
  ...
  const pattern = (validations as any)[metadata.isoCode as keyof typeof validations][
    Number(strict)
  ];
  return new RegExp(pattern).test(
    [metadata.areaCode, metadata.phoneNumber].filter(Boolean).join(''),
  );
};
...
const getDefaultISO2Code = () => {
  ...
  return (
    timezones[Intl.DateTimeFormat().resolvedOptions().timeZone as keyof typeof timezones] ||
    '' ||
    'us'
  );
};
...
const CustomPhoneInput = ({
  ...
  const defaultValueState =
    defaultValue || (countries.find(([iso]) => iso === defaultMetadata?.[0])?.[2] as string);
  ...
  const valueRef = useRef<string>('+82');
  ...
  const metadata = useMemo(() => {
    ...
    if (
      countriesList.find(([iso]) => iso === calculatedMetadata?.[0] || iso === defaultMetadata?.[0])
    ) {
  ...
  const setFieldValue = useCallback(
    (value: PhoneNumber) => {
      if (formInstance) {
        const namePath = [];
        const formName = (formContext as any)?.name || '';
  ...
  const onChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      if (event.target.value.substring(0, valueRef.current.length) === valueRef.current) {
        ...
        handleChange(
          { ...phoneMetadata, valid: (strict: boolean) => checkValidity(phoneMetadata, strict) },
          event,
        );
  ...
  return (
    <div className='ant-phone-input-wrapper' ref={(node) => setMinWidth(node?.offsetWidth || 0)}>
      <Select
        className='w-full mb-1 h-[39px]'
        suffixIcon={null}
        value={selectValue}
        ...
        onSelect={(selectedOption, { key: mask }) => {
          ...
          valueRef.current = formattedNumber;
      ...
      >
        {countriesList.map(([iso, name, dial, mask]) => (
          <Select.Option
            ...
            children={
              <div className='ant-phone-input-select-item'>
