import { createAdvertisingTypeApi, editAdvertisingTypeApi } from '@/apis/board/board.api';
...
import { adminAdvertisingTypeListKeys } from '../AdminAdvertysingTypeListPage';
...
export default function EditDistrictModal({
  ...
  const { mutate: muteAddAdvertising } = useMutation({
    mutationFn: (data: EditDistrictInput) => createAdvertisingTypeApi(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: adminAdvertisingTypeListKeys.lists() });
      setIsOpen(false);
      form.resetFields();
  ...
  const { mutate: muteEditAdvertising } = useMutation({
    mutationFn: (data: EditDistrictInput) => editAdvertisingTypeApi(data, initialValue?.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: adminAdvertisingTypeListKeys.lists() });
      setIsOpen(false);
      form.resetFields();
  ...
  return (
    ...
    >
      <Form
        ...
        onFinish={(data) => {
          if (initialValue?.id) muteEditAdvertising(data);
          else muteAddAdvertising(data);
        }}
        autoComplete='off'
        colon={false}
        form={form}
